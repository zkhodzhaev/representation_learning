...
def mlp_main(pkl_file, extraction_option, random_state):
    ...
    clf = MLPClassifier(hidden_layer_sizes=best_hidden_layer_sizes, activation=best_activation, random_state=random_state).fit(data['X'][data['idx_train']], y_train)
    ...
    print(f'CounterFactual Fairness (Unfairness Mean): {counterfactual_fairness_metric}')
    print(f'Robustness Score (Instability Mean): {robustness_score_metric}')
    print(f'Parity (Delta PS): {parity} | Equality (Delta EO): {equality}')
    print('------------------------------------------')

    return auc_roc_test, f1_s, counterfactual_fairness_metric, robustness_score_metric, parity, equality

# Modify the summary section to include new metrics
for pkl_file in pkl_file_to_sens_idx.keys():
    for extraction_option in ['PCA', 't-SNE', None]:
        ...
        for random_seed in range(1, 6):
            ...
            results = mlp_main(pkl_file, extraction_option, random_seed)
